...
<head>
<meta charset="UTF-8">
<title>Veille tech — Semaine {week}</title>
<style>
  body {{
...
<body>
<h1>Veille tech — Semaine {week}</h1>
<p style="color:#666;font-size:.9rem">{len(top)} articles retenus sur
{len(scored_items)} collectés — {len(poc_items)} idées de POC</p>
"""
    ...
    for s in top:
        score_cls = "high" if s.relevance >= 8 else ""
        tags_html = "".join(f'<span class="tag">{t}</span>' for t in s.tags)
        poc_html = f'<div class="poc">POC : {s.poc_idea}</div>' if s.poc_idea else ""
        html += f"""
<div class="item">
  <div class="item-title">
    <a href="{s.item.url}" target="_blank">{s.item.title}</a>
    <span class="score {score_cls}">{s.relevance:.0f}/10</span>
  </div>
  <div class="meta">{s.item.source} · {s.item.published[:10]} · {tags_html}</div>
  <div class="summary">{s.summary_fr}</div>
  <div style="font-size:.8rem;color:#6b7280;font-style:italic">{s.why_relevant}</div>
  {poc_html}
</div>"""
